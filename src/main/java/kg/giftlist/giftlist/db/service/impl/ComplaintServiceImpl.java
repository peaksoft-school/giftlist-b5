...
public class ComplaintServiceImpl {
    ...
    public SimpleResponse sendComplaintToWish(Long wishId, String text) {
        ...
        notification.setRecipientId(userRepo.findByRole().orElseThrow(() ->
                new NotFoundException("User by role ADMIN not found!")).getId());
        notification.setComplaintWish(complaint);
        user.addNotification(notification);
        notificationRepository.save(notification);
        return new SimpleResponse(
                "Отправлено! Спасибо, что сообщили нам об этом",
